...
def test_suggestions_click_turns_off_absence_mode_and_flashes(monkeypatch):
    import pygame
    pygame.init()
    import class_view
    ...
    class FakeButton(pygame.sprite.Sprite):
        def __init__(self, text):
            ...
            self.text_editor_active = False
            self.rect = pygame.Rect(0, 0, 10, 10)
            self.fade_from_1_to_0 = 0.0

        def update(self, screen):
            pass

    class FakeSuggestionsButton(FakeButton):
        def clicked(self, selected_desks):
            self.fade_from_1_to_0 = 1.0
    ...
    def fake_suggestions_button(pos, size, desks, comment_file, config_file, course):
        button = FakeSuggestionsButton("suggestions")
        created_buttons["suggestions"] = button
        return button
    ...
    def fake_handle_mouse_button_down(x, y, sprites, selected_desks):
        ...
        if len(click_sequence) == 1:
            return created_buttons["Absences"], selected_desks
        if len(click_sequence) == 2:
            return created_buttons["suggestions"].clicked(selected_desks)
        return class_view.icons.UnclickedDesk(), selected_desks
    ...
    monkeypatch.setattr(
        class_view.link_gui_backend,
        "setup",
        lambda *args: ({"assignments": {}, "gui_places": {}, "desks": [], "absent_students": set()},
                       (8, 7), "comments.txt", "positive.txt", "negative.txt"),
    )
    monkeypatch.setattr(
        class_view.link_gui_backend,
        "desks_from_seating_state",
    ...
    monkeypatch.setattr(class_view, "create_text_editor_comment_buttons", lambda *args: [])
    monkeypatch.setattr(class_view.icons, "Button", fake_button)
    monkeypatch.setattr(class_view.icons, "SuggestFocusButton", fake_suggestions_button)
    monkeypatch.setattr(class_view.events, "handle_mouse_button_down", fake_handle_mouse_button_down)
    monkeypatch.setattr(class_view.pygame.event, "get", fake_event_get)
    monkeypatch.setattr(class_view.pygame.display, "flip", lambda: None)
    ...
    assert created_buttons["Absences"].color_unclicked == created_buttons["Absences"].color_default
    assert created_buttons["suggestions"].fade_from_1_to_0 == 1.0
