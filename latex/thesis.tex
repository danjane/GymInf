...
The data flow diagram in figure \ref{dataflowdiagram}, on page \pageref{dataflowdiagram}, is a very high-level description of a possible system. To make concrete decisions about how to implement the TAT system it will help to define the expected inputs and outputs. In this section I will outline a basic backend based on files, then briefly explain the advantages of databases over files. An implementation based on files is simpler, but in the future I would expect the backend to handle updates via databases. I will finish this section by touching on the data privacy laws that apply in Geneva; in particular the system will use only a minimum amount of personal data.
...
\texttt{albert.enstn\\ngabriel.crmr\\nmarie.cr\\nrichard.fynmn}.
\end{tcolorbox}
This also seems a pertinent place to store the given name of each student. Currently, the user-id starts with the first name of the student. TODO
...
\item Create a virtual environment and add the necessary python modules. This can be done at the command line using
...
\item Copy the \texttt{GUI\_files} directory into the new \texttt{dist} folder.
\item Zip the necessary files into a single archive:
\begin{itemize}
