...
By "backend" I mean the data access layer of the system. By connecting the inputs and outputs we can assess what functionality will be required, and what functionality can be shared.

\

\texttt{https://tikz.net/product-architecture/}
...
\begin{tikzpicture}
    \node (box) [block] {Alphabetic seating plan}
    node (in1)     [left= 1.5cm of box]     {Class list}
    node (out1)    [right=1.5cm of box]    {Basic seating plan}
    (in1) edge[->] (box.west |- in1)
    (box.east |- out1) edge[->] (out1);
\end{tikzpicture}

\begin{tikzpicture}[node distance=30mm, every node/.style={draw, circle, text centered, minimum height=1.5cm, align=center, text width=1cm, font=\fontsize{6}{8}\selectfont}]
  % Nodes
  \node (students) {list of students};
  \node (process) [below right of=students, yshift=10mm, rectangle] {Process};
  \node (comments) [below left of=process, yshift=10mm,] {list of comments};
  \node (output) [right of=process] {comment table};

  % Arrows
  \draw[->] (students) -- (process);
  \draw[->] (comments) -- (process);
  \draw[->] (process) -- (output);
\end{tikzpicture}
