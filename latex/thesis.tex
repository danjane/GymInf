...
The process of developing complex IT systems involves various stages, from understanding client requirements to delivering and maintaining the final product. Traditionally, software development followed the waterfall method \cite{Amman16}, where each phase was executed sequentially: requirements gathering, design, implementation, testing, and deployment. However, this approach had several drawbacks, such as a long development cycle and limited client involvement until the final stages.  The latter (all too often) led to misunderstandings and mismatches between the final product and client expectations, resulting in costly redesigns that would have been cheaper if they had been identified earlier in the process. In this section I explore how industry practitioners tried to adapt the waterfall process.

\subsection{Reducing the Distance Between Clients and Developers}
As a response to costly overruns the \emph{"Agile"} methodologies emerged, emphasising iterative development and client collaboration \cite{Anaya18, Beck03}. The adoption of Agile methodologies aimed to bridge the gap between clients and developers throughout the process. Agile principles emphasise frequent communication, iterative development, and the delivery of working software in short cycles known as \emph{sprints}. This approach allowed clients to see progress regularly, provide feedback, and steer the development process in the right direction. 

Note that even with Agile principles it is still difficult to ensure that the delivered software precisely meets the client's needs. Further, as G. Lee and W. Xia pointed out in 2010, much of the alleged advantages of Agile development are anecdotal rather than scientifically proven \cite{LX10}. They find that there is a difference between being able to quickly respond to a particular change in user requirement (response efficiency), compared to being able to respond to as many changes as possible (response extensiveness).

The shorter sprints and early involvement of clients may also have led to developers becoming used to shipping beta versions of their software, rather than aiming for a final version.
...
\end{tabular}
\end{center}
Surprisingly there really is no difference as far as the metrics are concerned! Rather than being disheartened notice that the backend has a strong testing framework, making changes relatively risk free. Further, we might hope that I am becoming better at naturally and more completely refactoring as I code. Finally, recent research by A. Santos et al has shown that \emph{"[small projects] tend to provide less optimistic results for TDD"} and that TDD is more effective for teams.\cite{SVDU21}
