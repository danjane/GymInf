...
\begin{tikzpicture}[node distance=18mm, every node/.style={draw, circle, text centered, minimum height=12mm, align=center, text width=1cm, font=\fontsize{6}{8}\selectfont}]
  % Nodes
  \node (students) {list of students};
  \node (process) [below right of=students, yshift=5mm, xshift=5mm, rectangle] {shred comments};
  \node (comments) [below left of=process, yshift=5mm,xshift=-5mm] {raw comments as text };
  \node (comment_table) [right of=process] {comment table};
  \node (analyze) [right of=comment_table, rectangle] {analyze comments};
  \node (data) [right of=analyze] {student data};

  % Arrows
  \draw[->] (students) -- (process);
  \draw[->] (comments) -- (process);
  \draw[->] (process) -- (comment_table);
